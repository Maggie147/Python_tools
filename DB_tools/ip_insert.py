...
def main():
    ...
    with open(fpath, 'r') as file:
        ...
        print "insert mysql[ip_threats]..."
        lines = file.readlines()
        for item in lines:
            info  = item.split('|')
            try:
                sql = "insert into ip_threats(Id, IP, Time, Src, Type) values(null, '%s', '%s', '%d','%d')" % (\
                ...
                # failed_item = {'IP':info[0], 'Time':info[1], 'Src':info[2], 'Type':info[3]}
