...
class MailblackCkeck(object):
    ...
    def mail_black_check(self, start):
        ...
        for mailInfo in mail_infos:                                                                  # mailInfo[0], mailInfo[1], mailInfo[2], mailInfo[3] = Id, srcip , dstip, content
            ...
            #dstip_result = self.ip_search('ip_threats', "96.81.131.84")                               # for test
            if dstip_result:
            ...
            if domains:
                for domain_tmp in domains:
                    ...
                    try:
                        ...
                        domain_result = self.domain_search('domain_threats', "021id.net")                # for test
                        if not domain_result:
                            # domain_result = self.domain_search('domain_threats', domain[1].strip()+domain[2].strip())
                            domain_result = self.domain_search('domain_threats', "021id.net")             # for test
                        if domain_result:
                            if blackInfo['ContResult']:
                                blackInfo['ContResult'] = str(blackInfo['ContResult'])+';'+ str(domain_tmp[1])+'_'+str(domain_result['Src'])+'_'+ str(domain_result['Type'])+'_'+ str(domain_result['Time'])
                            else:
                                blackInfo['ContResult'] = str(domain_tmp[1])+'_'+str(domain_result['Src'])+'_'+ str(domain_result['Type'])+'_'+ str(domain_result['Time'])
                    except Exception as e:
